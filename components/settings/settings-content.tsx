...
import { User as UserIcon, Mail, Sparkles, Save, Crown, ExternalLink, Plus, Edit, Trash2, X } from "lucide-react"
...
export function SettingsContent({ user }: SettingsContentProps) {
  ...
  const allAvailableModels = [
    ...availableModels,
    ...customModels.filter(m => m.provider === provider).map(m => ({
      id: m.modelId,
      name: m.name,
      provider: m.provider,
  ...
  useEffect(() => {
    ...
    const loadConfig = async () => {
      setLoading(true)
      try {
        const [configResponse, subscriptionResponse, customModelsResponse] = await Promise.all([
          fetch("/api/ai-config"),
        ...
        <div className="space-y-6">
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={model} onValueChange={setModel}>
                      ...
                      <SelectContent>
                        ...
                        {customModels.filter(m => m.provider === provider).length > 0 && (
                          <>
                    ...
                    {!apiKey && !subscriptionStatus?.hasActiveSubscription && (
                      <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-md border border-red-200 dark:border-red-800">
                        <p className="text-xs text-red-800 dark:text-red-200 mb-2">
                          <strong>⚠️ Atenção:</strong> Para usar a chave de IA do servidor, você precisa de uma assinatura ativa.
                        </p>
                    ...
                    {subscriptionStatus?.hasActiveSubscription && (
                      <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-md border border-green-200 dark:border-green-800">
                        <p className="text-xs text-green-800 dark:text-green-200 flex items-center gap-2">
                          ...
                          <span>Você tem uma assinatura ativa e pode usar a chave do servidor.</span>
                        </p>
                      </div>
                    )}
                    {apiKey && (
                      <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-md border border-blue-200 dark:border-blue-800">
                        <p className="text-xs text-blue-800 dark:text-blue-200">
                          <strong>✓ Sua chave configurada:</strong> Você está usando sua própria chave de API. 
                          Isso permite controle total sobre custos e limites, sem necessidade de assinatura.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {customModels.filter(m => m.provider === provider).length > 0 && (
                    <div className="pt-4 border-t">
                      <Label className="mb-2 block">Modelos Personalizados</Label>
                      <div className="space-y-2">
                        {customModels
                          .filter(m => m.provider === provider)
                          .map((m) => (
                            ...
                            >
                              ...
                              <div className="flex gap-1">
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={async () => {
                                    if (confirm("Tem certeza que deseja remover este modelo personalizado?")) {
                                      try {
                                        const response = await fetch(`/api/custom-models?id=${m.id}`, {
                                          method: "DELETE",
                                        })
                                        if (response.ok) {
                                          setCustomModels(customModels.filter(cm => cm.id !== m.id))
          ...
          <Dialog open={showCustomModelDialog} onOpenChange={setShowCustomModelDialog}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>
                  {editingModel?.id ? "Editar Modelo Personalizado" : "Adicionar Modelo Personalizado"}
                </DialogTitle>
                <DialogDescription>
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="custom-model-description">Descrição (Opcional)</Label>
                  <Textarea
                  ...
                  <Button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (!response.ok) {
                          const error = await response.json()
                          throw new Error(error.error || "Erro ao salvar modelo")
                        }

                        // Recarregar modelos personalizados
                        const modelsResponse = await fetch("/api/custom-models")
              ...
              <div className="space-y-4">
                <div className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                  ...
                  <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
                    ...
                    e sistemas de design, tudo alimentado por Inteligência Artificial de última geração.
                  </p>
                </div>
